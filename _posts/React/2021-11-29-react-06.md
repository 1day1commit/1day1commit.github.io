---
layout: post
title: '[React 공식문서] Component 와 Props'
...
또한 **ES6 class**를 사용하여 컴포넌트를 정의할 수도 있습니다.
...
props의 이름은 하용될 context가 아닌 컴포넌트 자체의 관점에서 짓는 것을 권장합니다.
...
처음에는 컴포넌트를 추출하는 작업이 지루해 보일 수 잇습니다. 하지만 재사용 가능한 컴포넌트를 만들어 놓는 것은 더 큰 앱에서 작업할 때 두각을 나타냅니다. UI 일부가 여러 번 사용되거나 (`Button`, `Panel`, `Avatar`), UI 일부가 자체적으로 복잡한 (`App`, `FeedStory`, `Comment`) 경우에는 별도의 컴포넌트로 만드는 게 좋습니다.
